...
public class InstructionRegistry {
    ...
    private final class Entity {
        ...
        private final ByteCodeInstruction action;
    ...
    public InstructionRegistry() {
        ...
        // operations between primitives on the instruction
        ...
        this.register(DupAction.class, "dup"); // duplicate head of instruction
    ...
    public boolean invoke(Context context, CodeAttribute.CodeItem item, int[] data) {
        Entity entity = this.actionMap.get(item.getId());
        if(entity != null) {
            return entity.action.handle(entity.type, data, item.getPc(), context);
        }
    ...
    private void register(Class<? extends ByteCodeInstruction> clazz, String tag, VariableStorage.PrimitiveType... types) {
        register(false, clazz, tag, types);
    }


    private void register(boolean reverse, Class<? extends ByteCodeInstruction> clazz, String tag, VariableStorage.PrimitiveType... types) {
        try {
            ByteCodeInstruction action = clazz.newInstance();
            if(types.length == 0) {
